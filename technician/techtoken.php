...
<?php
$m=$r['sid'];
$a=$conn->prepare("select * from sales where sid=?");
$arr=array($m);
$a->execute($arr);
while($x=$a->fetch()){
?>
<tr height="50px;"> <td style="text-align:center; "><?php echo $x['product']?></td>
...
<td><form action="setcheck.php" method="post">
<input type="hidden" name="tid" value="<?php echo $r['tid']?>" />
<input type="submit" name="submit" value="set checklist" />
</form></td></tr>
<?php
